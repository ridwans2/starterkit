...
| 35 | Celah yang diketahui dari alarm #34: scan hanya membaca **method**; file yang di-revert sampai ke bentuk **properti statis** lolos dari scan (dipegang `LabelPainelLewatGetterTest`, 43 kasus). Medido: `git checkout HEAD -- ConfiguracoesDoKit.php` → kasus scan hijau, kontrol positif MERAH | ✅ tertutup — `git checkout HEAD --` pada file bergetter UNCOMMITTED MENGHAPUS getter (terjadi nyata, lihat HASIL BARIS 36–40) |
| 36 | Aset tahan-update: `app/Console/Commands/I18nExtract.php` — kamus = `lang/pt_BR.json` dibalik (satu pemilik), 3 konteks UI, `--report`/`--candidatos`/`--fail`, tabrakan kamus = stop | ✅ |
| 37 | Guard `tests/Feature/I18nExtractTest.php` — fixture byte-exact (4 bentuk), idempotensi, collision guard | ✅ |
| 38 | Area 2: kamus +46 (pt_BR 53→99, id 20→66), extractor apply **72 literal di 28 file** | ✅ code-done · oracle Kit **berjalan** |
| 39 | Sisa terukur di konteks UI (`--candidatos`, default allowlist): **582 kemunculan / 425 distinct** (turun dari 1.098 jalur render) | belum |
| 40 | Celah extractor yang terukur: argumen pertama `X::make('Label')` (Tab/Section/StatPlus) BUKAN konteks UI → `Tab::make('E-mail')`, `Section::make('Usuários e acesso')`, `StatPlus::make('Papéis', …)` masih PT. Butuh allowlist kelas + aturan arg-1 | belum |
| 41 | Dikeluarkan dari kamus dengan alasan: `'Administração'`/`'IA'` = **kunci grup navigasi** (4 `navigationGroup` statis). Menerjemahkan hanya sisi `->group()` memecah resource ke dua grup berbeda saat `en` | belum (perlu getter grup, pola baris 16) |
| 42 | Dashboard `/admin` di browser: 74 baris teks, **9 masih beraksen PT** — semuanya bentuk #40/#41/fragmen interpolatif | ✅ terukur |
| 43 | Locale `id` jadi kelas satu: `laravel-lang/common` (dev) + `php artisan lang:add id` → `lang/id/{actions,auth,http-statuses,pagination,passwords,validation}.php` + `id.json` 297 kunci. Medido: `__("auth.failed")` di `id` = "Identitas tersebut tidak cocok dengan data kami." | ✅ |
| 44 | Extractor: allowlist konteks diperluas (`navigationLabel`, `modelLabel`, `pluralModelLabel`, `groupLabel`, `navigationGroup`, `successNotificationTitle`, `failureNotificationTitle`, `confirm`, `notice`) + jalur `app/Providers` + `app/Livewire` + aturan **arg-1** untuk `X::make()` (Section/Tab/Text/StatPlus) | ✅ |
| 45 | Guard layar: `tests/Feature/NenhumRotuloPortuguesNaTelaTest.php` — render semua rota GET tanpa parameter sebagai `master_global` di locale `en`, larang SEMUA nilai `lang/pt_BR.json`. 48 dari 84 tela render; temuan turun 27→12 | ✅ |
| 46 | Seed/fabrik English: 12 penggantian (nama tenant demo, 3 user, 4 proyek, 2 `descricao` agen, dashboard default `Padrão`→`Default`) + `KIT_TENANCY_LABEL*` dan **Settings tersimpan** `rotulo_da_organizacao` → Organization(s). Diterapkan via `db:seed` (semua `updateOrCreate`/`firstOrCreate` → non-destruktif, admin tidak disentuh) | ✅ |
| 47 | **JEBAKAN BARU yang terukur**: `__()` di registrasi provider dievaluasi di **BOOT** (locale config `en`, sebelum pin `LocalizacaoProvider`) → nilai beku. `TelaDePapeisTest` jatuh (`'Papel'` vs `'Role'`) persis karena ini. Konsumen yang menerima string mentah lalu menerjemahkan sendiri (Shield, Filament `translateLabel`) harus dapat **literal English mentah**, bukan `__()` | ✅ terbukti, dicegah guard #48 |
| 48 | Grup navigasi = **identificador**, bukan copy: 18 titik jadi string English mentah (7 getter `getNavigationGroup()` + 11 provider). Alasannya terukur: halaman milik plugin (`->navigationGroup()` di provider) TIDAK punya jalur per-request, jadi satu sisi `__()` + sisi lain mentah memecah grup yang sama jadi dua di pt_BR | ⚠️ menunggu hasil Kit subset |
| 49 | Sisa yang diketahui dari audit #45: label field Blade (`E-mail` di `meu-perfil`), nilai `Text::make()`/vendor PT-only (`Backups`, `Pacotes do Composer` → butuh `lang/vendor/*/en/`), fragmen interpolatif, dan 2 prompt `INSTRUCOES` agen (sengaja: input fungsional AI, bukan copy) | belum |
| 50 | Guard layar jadi RATCHET DE HIMPUNAN + baseline terukur (13 tela, dari JSON dump `medida.txt`), plus **kontrol positif** `textodeTela()` (payload `wire:`/`<template>`/`<style>` harus tidak dihitung). Feature **72 passed**, phpstan 0, pint bersih | ✅ |
| 51 | Label Shield per-request: `RoleResource::{getModelLabel,getPluralModelLabel,getNavigationLabel}` menutup injeksi eager do plugin — `TelaDePapeisTest` **43 passed** membuktikan `getRecordTitle(null) === 'Papel'` di pt_BR dan 'Role' di en | ✅ |
| 52 | Status user: `rotuloDaSituacao()` = `__(chaveDaSituacao())`; cor agora casa sobre a chave English crua. Antes, `corDaSituacao()` casava sobre o rótulo traduzido → cor quebraria só em pt_BR | ✅ |
| 53 | Bug que EU criei e os geradores pegaram (cada um tem agora uma guarda): `__()` eager em provider (TelaDePapeis vermelho) · `DOMDocument::loadHTML` bisu em HTML5 Livewire (guard verde sem ler nada) · literal `\x27` escrito no código (14 tela vermelhos) · `TableColumn::label()` inexistente (phpstan) · `*/` dentro de docblock (parse error) · regex em `php -r` com quoting Windows → pakai file | ✅ tercatat |
| 54 | Verifikasi browser nyata (iframe same-origin, `innerText` = yang benar-benar tampil): `/admin` 30 baris **0 PT** · `/admin/users` 78 baris **0 PT asli** · `/admin/convites/create` 4 kalimat prosa PT · lock screen penuh English | ✅ sebagian |
| 55 | Sisa nyata terukur: prosa `->description()`/`->hint()` di create-convites/users/roles/meu-perfil + landing `/` · `lang/vendor/*/en/` ( paket PT-only: `Backups`, `Pacotes do Composer`) · Fase 7 kolom/class/slug · 2 prompt agen | belum |
| 56 | **Área 3 = PROSA**: +126 frasa ke kamus (`i18n:extract --candidatos` → filter "frase inteira" → terjemah per INDEKS supaya PT tidak pernah diketik ulang) → **316 titik __() baru di app/ (74 berkas berubah termasuk overlay)**. Kamus: 317 (pt_BR) / 509 (id) kunci. Guard layar turun dari **13 tela jadi 2** | ✅ |
| 57 | Allowlist do extractor ampliada: `helperText`, `inlineHelp`, `placeholder`, `message`, `errorMessage`, `confirmDescription`, `confirmHeading`, `emptyStateButton` — `helperText` é o motivo de a prosa ter sobrado | ✅ |
| 58 | **Jebakan terukur**: argumento de atributo PHP é EXPRESSÃO CONSTANTE — `__()` dentro de `#[Validate(message: [...])]` = `Constant expression contains invalid operations`. Extractor agora (a) pula `#[...]` e (b) recusa escrever resultado que não passa em `php -l` | ✅ |
| 59 | Heading de widget (8× `protected ?string $heading`) → `getHeading(): ?string` por request. Assinaturas conferidas no vendor: `StatsOverviewWidget::getHeading()` é protected e `ChartWidget::getHeading()` é pública `string|Htmlable|null` | ✅ |
| 60 | `__()` devolve `array|string` e bateu em parâmetro `string|null` (`BreakdownItem::description`) → cast `(string)` no ponto. phpstan 0 erros | ✅ |
| 61 | Browser: topbar + sidebar `/admin` 100% English, dialog notificação English, `/app` e `/infra/execucoes-ia` 0 linhas PT. O que resta no dashboard é DADO GRAVADO no banco (título de dashboard dinâmico, label de filtro salvo) — a string não existe no código | ✅ terukur |
| 62 | Regressi dari run Kit penuh: `CitacoesDeCodigoTest` CT-26 (sitasi `Arquivo.php:123` di komentar jadi basi karena baris bergeser) + `CustomizadorDaInstalacaoTest` (detail menunggu akhir run) | ✅ CT-26 · lihat 64 |
| 63 | CT-26 diperbaiki: 3 sitasi basi dipindah ke baris declaration yang sebenarnya — `User.php:papeisEmQualquerContexto:683→731`, `canAccessTenant:789-809→806-861`, `getTenants:775-782→792-799` (dua terakhir di docblock `tests/Tenancy/LinkDoPainelDaOrganizacaoTest.php:590`) | ✅ |
| 64 | **Dua merah Kit BUKAN dari kerja ini, terbukti lewat git**: `MysqlNoDockerTest` CT-14/CT-22 — `docker-compose.yml` `name: raja-starter-kit` datang dari commit user `560dd0c "new"`, dan guard menuntut `starter-kit` di DUA sumber (floor file + `.env.example`); `CustomizadorDaInstalacaoTest` "senha padrão" — afirmasi `config('kit.admin.password')==='password'` sedangkan `.env` lokal `KIT_ADMIN_PASSWORD="sheila"`. Keduanya keputusan user, tidak saya sentuh | belum |
| 65 | CT-25 (`SiteDeDocumentacaoTest`) tripwire jumlah arquivo de teste di README: 180 → **184** (4 guard baru sesi ini), `README.md` + `README.en.md` | ✅ |
| 66 | **Kebutaan oracle yang ketemu**: guard layar hanya mengenali frase yang SUDAH ada di `lang/pt_BR.json` → prosa PT yang belum pernah dikatalogi tak terlihat. Buktinya 4 defek nyata lolos dalam keadaan hijau: `Section::make('Vinculos')`, `->label('Convidar em massa')`, `$heading = 'Convites por situação'`, `'Organizacoes'/'Papeis'` (PT tanpa aksen). Guard kini punya DIMENSI DUA: léxico `$padraoPt` (kata utuh) dijadikan **ratchet de himpunan** (sebelumnya cuma count) dengan baseline terukur per tela | ✅ |
| 67 | Presisi guard (supaya merah itu bisa dibaca, bukan sekadar ada): `(?<!\p{L})…(?!\p{L})` membunuh false positive `'Aceito'∈'Aceitos'`, `'Conta'∈'Contains'`, `'Erro'∈'Errors'`; rute non-HTML (endpoint JSON `ai-tasks/data`) di-skip; pesan merah kini mencetak JANELA ±48 karakter di sekitar frase | ✅ |
| 68 | Onda de prosa: `i18n:extract` jalan sebagai DRIVER migrasi (kamus = overlay yang saya tambahkan; PT tidak pernah diketik ulang, selalu diambil dari fila) → **116+2 literais di 40 arquivo**, `lang/pt_BR.json` 317 → **470 kunci**, `lang/id.json` → 537. Sisa yang dijanjikan guard: prosa Breezy `meu-perfil` (3 painel), paragrafo 2FA/recuperacao `configuracoes-da-aplicacao`, nama permission dari DB, dan `/` 'organização'/'padrão'/'âmbar' (label konfigurasi + dado) | sebagian |
| 69 | Kasus yang ekstrator TIDAK capai, diperbaiki tangan: Blade (`errors/403.blade.php` `$auth['Conta']` — kunci array yang jadi label; `auth/conta-indisponivel.blade.php` judul), literal duplo-quote interpolado (`"{$ajuda} Até {$max} MB, …"`, `"O arquivo passa de {$max} MB."`), `sprintf` 3-posisi → kunci berset `:exceptions/:emails/:exports`, dan 3 fragmento `'… nos últimos '.$n.' dias'` (inggris+kata PT) → kunci tunggal `:days` | ✅ |
| 70 | `METODOS_DE_UI` ekstrator ditambah `body` + `subtitle` — `->body('Peça um convite novo…')` lolos persis karena `body` tidak dianggap posisi teks. Ini alasan struktural kenapa "sudah migrasi" bukan berarti "selesai" | ✅ |
| 71 | Grup navigasi dari CONFIG plugin masih PT (`filament-authentication-log.group = 'Trilhas'`, `filament-jobs-monitor.navigation_group = 'Observabilidade'`) → sidebar `/infra` PT **dan** grupnya pecah dari provider yang sudah English. Diganti ke `'Trails'`/`'Observability'`; kedua file config tidak ada di `CAMINHOS_DO_KIT` = imun struktural | ✅ |
| 72 | Status `Convite`: `situacao()` TETAP identitas PT (Kit menegaskan `'Aceito'/'Pendente'/'Recusado'/'Expirado'` byte-per-byte), display pindah ke `Convite::ROTULOS_DA_SITUACAO` + `rotuloDaSituacao()` dipakai kolom `/admin`, kolom `/app` dan legenda donut → `en` Inggris, `pt_BR` kembali ke string yang sama | ✅ |
| 73 | Jebakan yang SAYA buat sendiri, tercetak di sini supaya tidak diulang: (a) regex guloso memasangkan kunci EN ke PT yang SALAH (`'Ask whoever administers…'` dapat badan `'Sua conta foi criada…'`) — ketahuan saat baca hasil, dua kunci dipisah; (b) `Section::make('Memberships')` tanpa `__()` = CT-53 merah, karena heading Section TIDAK di-auto-translate oleh `translateLabel()` — yang auto hanyalah `->label()`; (c) script tambah kunci hanya cek tabrakan KUNCI, bukan NILAI: dua EN key dengan satu nilai PT membuat `i18n:extract` abort (itu desainnya), dan ketahuan baru saat `--report` | ✅ |
| 74 | Trade-off satu-kunci-Inggris: `'Accepted'` dipakai status (PT `'Aceito'`) dan tab (PT `'Aceitos'`), `'Created at'` untuk `'Criado em'` dan `'Criada em'` → bahasa Inggris benar, pt_BR kehilangan plural/gender. Diukur dulu: tidak ada test yang menegaskan `'Aceitos'`/`'Criada em'` | ✅ |
| 75 | Prosa dos **e-mails** (não é tela, é o que a pessoa lê na caixa de entrada): `ConviteDeAcesso` (2 botões, linha de validade, linha de aviso, salutation) e `PrimeiroAcessoSocial` (greeting + 4 linhas) → 12 chaves novas com o PT copiado do próprio arquivo, porque `tests/Kit` afirma sobre `Aceitar convite`/`Atenciosamente,` | ✅ |
| 76 | Prosa que chega como **argumento de helper do projeto** (`ConfiguracoesDoKit::arquivo($campo, $rotulo, $ajuda)`, 3 chamadas) e label interpolado `"Habilitar botão do {$provedor->rotulo()}"` → chaves com `:provider`. O extrator não alcança nenhum dos dois de propósito: ele conhece a UI do Filament, não a assinatura de helper deste arquivo | ✅ |
| 77 | **Fase 5 — máscara de data segue o idioma**: classe nova `App\Support\Formatos` (arquivo do projeto = imune) + `data_hora_segundos` em `config/localization.php` + os três defaults aplicados num lugar só, `ConfiguraFilamentGlobal::configuraTable()`. 25 máscaras `'d/m/Y[ H:i[:s]]'` em 22 arquivos viraram leitura de config; `grep -c d/m/Y app/` = 1 (só o docblock que explica). Guard em `IdiomaDoProjetoTest` com dataset dos 3 idiomas: pt_BR devolve EXATAMENTE a máscara que estava cravada | ✅ |
| 78 | **Dois bug meus que só apareceram porque a guarda cresceu**: (a) `app()->locale()` não existe (`getLocale()`) e (b) faltou o `use App\Support\Formatos;` no provider → TODA tabela do /admin devolvia 500. Antes desta linha, o efeito visível era "o guarda renderizou só 22 telas" — verde-por-ignorado. A guarda agora NOMEIA toda tela que devolve 5xx na mensagem | ✅ |
| 79 | Widget do perfil em Blade (`definir-senha-por-email.blade.php`: heading, description, nota e botão) — `Kit/DefinirSenhaPorEmailTest` e `BrowserTenancy/CapturaDeArteTest` afirmam sobre o PT dessas 4 strings, então o overlay devolve byte por byte. Blade compila limpo (`view:cache`) antes de escrever | ✅ |
| 80 | Overlay: `lang/pt_BR.json` 317 → **491** chaves, `lang/id.json` → **544**. `i18n:extract --report` hoje responde "nada em português para aplicar" — a superfície CONHECIDA pelo dicionário está toda migrada; o que resta é o que o dicionário ainda não conhece, e é a guarda de tela que aponta | ✅ |
| 81 | Extrator ficou **tolerante a acento**: dicionário ganha a variante sem acento de cada valor PT (só quando não colide com chave exata) → 782 entradas; caso novo em `I18nExtractTest` com `'Ja configurada — em branco mantem'`. Também `body` e `subtitle` entraram em `METODOS_DE_UI` — era por isso que o corpo de uma notificação/infolist passava | ✅ |
| 82 | Sisa medido depois desta onda (registrado no baseline da guarda, nao escondido): nama permission yang datang dari **DB** (`/admin/shield/roles/create`), label tenant/dashboard yang **tersimpan** di Settings/rows, dan fragmen yang belum pernah masuk kamus di luar 25 titik. `lang/id.json` sengaja TIDAK diisi untuk prosa baru — fallback-nya Inggris, bukan PT | belum |
| 83 | Halaman error 404/419/500/503 pindah ke literal Inggris + overlay (`Home`/`Back` dipakai ulang, tidak diduplikasi). Diperiksa dulu: TIDAK ada satu pun test (Kit/Feature) yang me-assert string PT halaman error, jadi migrasi aman. `view:cache` hijau | ✅ |
| 84 | `config/command-center.php`: 2 dari 8 `help` masih PT (sisa dari gelombang sebelumnya) → Inggris, sama seperti tetangganya. Tanpa overlay: file config proyek, label command center memang tidak diterjemahkan | ✅ |
| 85 | **Menu user** di 3 panel masih `Meu perfil` (`userMenuLabel` di AdminPanelProvider:225 / AppPanelProvider:342 / InfraPanelProvider:253) → `My profile` + overlay (`My profile` → `Meu perfil` / `Profil saya`). Breezy me-render label itu lewat `Action::label()`, jadi pt_BR tetap Portugal | ✅ |
| 86 | **Sidebar /infra**: `Lixeira` → `Recycle bin`, `Comandos` → `Commands`, label panel `Infraestrutura` → `__('Infrastructure')` (Paineis:260). Overlay pt_BR 523 → **539**, id 725 → **741** | ✅ |
| 87 | Sapuan browser lengkap: **69 URL, 68 ter-render** (1 timeout diulang dan hijau). Audit menu (user menu + sidebar + topbar, dibaca lewat `textContent` container `.fi-dropdown-panel/.fi-sidebar nav/.fi-topbar` karena `innerText` TIDAK menghitung elemen tersembunyi): **77 baris unik, 0 PT** di ketiga panel. Ini juga yang menemikan `Meu perfil`/`Lixeira` — keduanya lolos sapuan `innerText` | ✅ |
| 88 | Pelajaran oracle (bukti buta, bukan asumsi): (a) dropdown Filament ada di DOM tapi `display:none` → `innerText` layar membersihkannya, jadi "0 PT" dari sapuan layar saja TIDAK membuktikan menu user; (b) kata PT **tanpa aksen** (`Lixeira`, `Comandos`, `perfil`, `Meu`) lolos leksikon beraksen. Kontrol yang menyelamatkan: dump baris mentah, bukan hanya angka | ✅ |
| 89 | Still red→green after the ratchet: `NenhumRotuloPortuguesNaTelaTest` initially flagged `reativar` on `/admin/shield/roles/create`. Cause is the same family already accepted in the baseline for that screen (permission names humanized from `permissions.name`, whose identity is `Action::make('reativar')` + `User::reativar()` + policy `reativar`) — added as `reativar` next to `Reativar`, not silenced. Real fix is Fase 7 (rename the action/policy/permission identity) or a display map on Shield's option labels | ✅ |
| 90 | Suite status ronde ini: `tests/Feature` **77 passed / 168 assertions** (termasuk guard label + locale), `pint --dirty` passed, dev app masih hidup (`/` 200, `/admin` 302 → login). `tests/Kit` full BELUM dijalankan (stage-nya truncate MySQL yang sama dengan dev server) — masih utang verifikasi | belum |
| 91 | Sisa PT yang sudah diklasifikasi dan TIDAK akan "diperbaiki" sebagai copy: slug tenant `padrao` (+URL `/app/padrao`), slug halaman `meu-perfil`, channel log `autenticacao` + nama file `autenticacao-*.log`, nama permission Shield di DB. Semuanya identifier, bukan label → Fase 7 | belum |
| 92 | **Label permission Shield jadi Inggris di layar tanpa menyentuh identitas**: `filament-shield.localization.enabled => true` + `lang/{en,pt_BR,id}/rotulo_permissao.php` (chave = `Str::snake()` do nome; AS DUAS trilhas do resolvedor cobertas: afixo `desativar` e custom `aceitar_convite`). Medido na tela `en`: Aceitar=0 Recusar=0 Desativar=0 Reativar=0, com Accept/Decline invitation e Deactivate/Reactivate=1. `pt_BR` byte-identico ao texto antigo, `id` traduzido. Tahan `kit:update` (config do pacote e `lang/**` fora de `CAMINHOS_DO_KIT`) | ✅ |
| 93 | Baseline da guarda revertida de verdade: as 4 palavras de permissao + `reativar` SAIRAM de `$frasesJaConhecidas` e `NenhumRotuloPortuguesNaTelaTest` segue verde (2 passed / 9 assertions) | ✅ |
| 94 | `tests/Kit` penuh dijalankan terhadap **MySQL scratch `starterkit_kit`** (bukan `starterkit`) supaya `RefreshDatabase`/`migrate:fresh` tidak menghapus data dev; env shell menang karena Dotenv immutable. Log: `.raja/temp/kit-full.txt` | belum |
| 95 | Pemindai baru `.raja/temp/panen-literal-pt.php` (token_get_all, bukan posisi-known ekstraktor) menghasilkan 159 literal PT. Klasifikasi: mayoritas output CLI `kit:*` dan pesan log — memang PT dan di-assert byte-for-byte oleh `KitInfoTest`/`CustomizadorDaInstalacaoTest`, jadi TIDAK disentuh. Yang posisi-layar dan aman: 4 string | ✅ |
| 96 | 4 string layar dipindah ke English-first: `EmptyState` 'Nenhum usuário tem este papel' (RoleResource:510), 'Log de acesso não disponível nesta instalação', 'Ninguém desta organização entrou ainda' (OrganizacaoStats), 'Origem não identificada' (OrganizacaoUltimosAcessos) → `__()` + overlay (pt_BR 540→544, id 742→746). Feature 77 passed, pint passed | ✅ |
| 97 | kelas yang MASIH terbuka dari hasil pemindai: pesan penolakan yang hidup di `private const MOTIVO_DA_NEGACAO` (`ConviteResource:47`, `ProjetoResource`, `UserResource:54/56`) — const PHP tidak boleh memanggil `__()`, jadi migrasinya butuh ubah const→method di titik pakai; dan fallback `config('kit.tenancy.label', 'Organização')` yang hari ini tidak terpakai karena `.env` sudah `KIT_TENANCY_LABEL="Organization"` (yang masih PT di `.env` cuma `KIT_TENANCY_SLUG="organisasi"` → identifier, Fase 7) | belum |
| 98 | Kelas const→method selesai: 3 pesan penolakan (`ConviteResource`, 2x `App/Users/UserResource`) pindah ke `__()` + overlay (pt_BR 544→**547**, id 746→**749**), 0 referensi `MOTIVO_DA_NEGACAO` tersisa, `tests/Feature` 77 passed | ✅ |
| 99 | **`tests/Kit` penuh pertama kali tuntas** (MySQL scratch `starterkit_kit`, RC=1): 4 file merah. CT-14/CT-22 (`MysqlNoDockerTest`, rebrand compose user) + `CustomizadorDaInstalacaoTest` senha (`.env`) = pre-existing terdokumentasi. `ConfiguracoesDoKitTest` "devolve a config ao que os arquivos dizem" merah karena `.env` lokal (`services.google.client_id`: `''` vs `null`) — juga lingkungan, bukan kode. Yang keempat = regresi nyata pekerjaan i18n, diperbaiki di baris 100 | ✅ |
| 100 | Regresi itu: overlay pt_BR menulis placeholder `:formatos` padahal kodenya mengirim `['formats' => ...]`, jadi pesan tampil mentah `SVG não é aceito. Envie :formatos.` dan 3 kasus `UploadLimiteETiposTest [CT-07b]` merah. Diganti ke `:formats`; KEDUA kamus dipindai dan hanya ada satu offender. Verifikasi ulang `UploadLimiteETiposTest`+`ConfiguracoesDoKitTest`: **80 passed / 1 failed** (yang 1 = google client_id `.env`) | ✅ |
| 101 | Varredura locale `id` di SEMUA rota (`.raja/temp/varai-id.php`, in-process, strip_tags termasuk konten tersembunyi): 70 rute, **40 layar ter-render dan bersih PT**, sisanya balas 3xx/4xx (rute panel lain/sesi). Hanya SATU layar masih Portugal di `id`: `/admin/shield/roles/create`, 2 potongan = 'Montar' dan 'Dashboard dinâmico' — label custom permission dari keluarga Shield yang baru dibuka (butuh kunci `montar`/`dashboard_dinamico` atau kunci nama-penuhnya di `lang/*/rotulo_permissao.php`). Inggris di `id` bukan temuan: itu fallback sah | ✅ |
| 102 | Dugaan "Montar = affix permission" TIDAK terbukti: kunci `montar`/`montar_dashboard_dinamico` saya tambahkan lalu diukur (`roles/create` di `id`: Montar=1, Pasang=0) → tidak dipakai, dan sudah **dihapus lagi** (kunci mati lebih buruk daripada utang yang tercatat). Kesimpulan: teks itu label permission `Dashboard dinâmico` yang lewat jalur `getLocalizedWidgetLabel`/heading entitas, BUKAN lewat `rotulo_permissao` — pintu masuk perbaikannya di label Resource/Page, dan `Dashboard dinâmico` tetap masuk ratchet layar itu | ✅ |
